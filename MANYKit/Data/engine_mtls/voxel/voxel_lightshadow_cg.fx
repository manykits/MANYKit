...
)
{
	...
	vertexTCoord3.rgb = ShineEmissive.rgb + LightAmbient_Dir.a * (ShineAmbient.rgb * LightAmbient_Dir.rgb + ShineDiffuse.rgb * LightDiffuse_Dir.rgb * max(dot(worldNormal, -LightWorldDVector_Dir.rgb), 0.0) +
...
)
{
	...
	{	
		...
		{
			shadowDepth += GetDepth(texCord, 0, 0);
		...
		// float brightness = dot(lastColor, float3(0.2126, 0.7152, 0.0722));
		// if (brightness > 1.0)
