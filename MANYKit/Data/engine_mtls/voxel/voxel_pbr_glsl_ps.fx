...
in vec2 vertexTCoord5;
in vec4 vertexTCoord6;
in vec4 vertexTCoord7;
in vec4 vertexTCoord8;
in vec2 vertexTCoord9;
...
void main()
{
	...
	vec2 texCoordCornor = vec2(vertexTCoord1.x, 1.0-vertexTCoord1.y);
	
	vec2 texCoordNormal = vec2(vertexTCoord2.x, 1.0-vertexTCoord2.y);
	vec2 texCoordMetric = vec2(vertexTCoord3.x, 1.0-vertexTCoord3.y);
	vec2 texCoordRoughness = vec2(vertexTCoord4.x, 1.0-vertexTCoord4.y);
	vec2 texCoordAO = vec2(vertexTCoord5.x, 1.0-vertexTCoord5.y);
	...
	{
		float metallic  = texture(SampleMetallic, texCoordMetric).r;
		float roughness = texture(SampleRoughness, texCoordRoughness).r;
		float ao        = texture(SampleAO, texCoordAO).r;
		vec3 albedo = pow(lastColor.rgb, vec3(2.2));
		vec3 WorldPos = vertexTCoord7.xyz;
		vec3 WorldNormal = vec3(vertexTCoord8.x, vertexTCoord8.y,vertexTCoord8.z);
		vec3 N = getNormalFromMap(SampleNormal, texCoordNormal, WorldPos, WorldNormal);
		...
		vec4 cornerColor = texture(SampleCorner, texCoordCornor);
		lastColor += (cornerColor*0.25); 
	
		lastColor.rgb = mix(FogColorHeight.rgb, lastColor.rgb, vertexTCoord5.x);
		lastColor.rgb = mix(FogColorDist.rgb, lastColor.rgb, vertexTCoord5.y);
