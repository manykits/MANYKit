...
void main() 
{
	...
	mediump vec3 worldPosition = (WMatrix * vec4(modelPosition, 1.0)).xyz;
	mediump vec3 worldNormal = normalize(mat3(WMatrix) * modelNormal);
	
	mediump vec3 viewVector = normalize(CameraWorldPosition.xyz - worldPosition);
	...
	mediump vec3 halfVector = normalize((viewVector - LightWorldDVector_Dir.xyz)/2.0);
	mediump float dotH = dot(worldNormal, halfVector);
