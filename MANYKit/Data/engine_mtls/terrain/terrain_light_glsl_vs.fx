...
void main()
{
	...
	vertexColor.rgb = ShineEmissive.rgb + LightAmbient_Dir.a * (ShineAmbient.rgb * LightAmbient_Dir.rgb + ShineDiffuse.rgb * LightDiffuse_Dir.rgb * max(dot(worldNormal, -LightWorldDVector_Dir.xyz), 0.0) + 0.0001 * ShineSpecular.rgb * LightSpecular_Dir.rgb * pow(max(dotH, 0.0), ShineSpecular.a*LightSpecular_Dir.a));
	...
	vertexColor.rgb += DoLight_Point_Diffuse(LightGroup[3].xyz, LightGroup[3].w, LightGroup[4].rgb, ShineDiffuse.rgb, worldPosition.xyz, worldNormal.xyz);
