...
out vec4 vertexTCoord2; // lightmodel
out vec4 vertexTCoord3;
out vec4 vertexTCoord4;
out vec4 vertexTCoord5; // light t; world pos x
out vec4 vertexTCoord6; // light b; world pos y
out vec4 vertexTCoord7; // light n; world pos z
...
void main()
{
	...
	//vec3 T = normalize(tangent - dot(tangent, modelNormal) * modelNormal);
    //vec3 B = cross(modelNormal, T);
	//mat3 mat = mat3(T, B, modelNormal);
	
	mat3 TBN = mat3(tangent, binormal, modelNormal);
	mat3 TBN_I = transpose(TBN);  

	// view
	vec3 cdir = normalize(CameraModelPosition.xyz - modelPosition.xyz);
	vertexTCoord3.xyz = TBN_I * cdir;

	// shadow
	vertexTCoord4 = ProjectPVBSMatrix_Dir * vec4(modelPosition, 1.0);

	vec3 ldir = -normalize(LightModelDVector_Dir.xyz);
	vertexTCoord2.xyz = TBN_I * ldir;
	...
	vertexTCoord5.xyz = TBN_I * lv0;
	vertexTCoord6.xyz = TBN_I * lv1;
	vertexTCoord7.xyz = TBN_I * lv2;
	...
	vertexTCoord0.z = fogValueHeight;
	vertexTCoord1.z = fogValueDist;
}