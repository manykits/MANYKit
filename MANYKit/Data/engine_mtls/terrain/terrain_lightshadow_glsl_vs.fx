...
vec3 DoLight_Diffuse(vec3 lightWorldPos, float lightRange, vec3 lightColor, vec3 shineDiffuse, vec3 vertexWorldPos, vec3 vertexWorldNormal, vec3 lightDir, float cosAngle)
{
	...
	vec3 cv = lightColor * shineDiffuse * max(0.0, dot(vertexWorldNormal, lightToVertex)) * max(0.0, (1.0 - squareDist / lightRange / lightRange) );
	
	float vertexCosAngle = dot(lightDir, lightToVertex);
	if (cosAngle > 0.0277)
	{
		return cv;
	}
...
void main()
{
	...
	vertexColor.rgb += DoLight_Diffuse(LightGroup[0].xyz, LightGroup[0].w, LightGroup[1].rgb, ShineDiffuse.rgb, worldPosition.xyz, worldNormal.xyz, LightGroup[2].xyz, LightGroup[2].w);
	vertexColor.rgb += DoLight_Diffuse(LightGroup[3].xyz, LightGroup[3].w, LightGroup[4].rgb, ShineDiffuse.rgb, worldPosition.xyz, worldNormal.xyz, LightGroup[5].xyz, LightGroup[5].w);
	vertexColor.rgb += DoLight_Diffuse(LightGroup[6].xyz, LightGroup[6].w, LightGroup[7].rgb, ShineDiffuse.rgb, worldPosition.xyz, worldNormal.xyz, LightGroup[8].xyz, LightGroup[8].w);
	...
	vertexTCoord2.x = fogValueHeight;
	vertexTCoord2.y = fogValueDist;
	
	vertexTCoord3 = ProjectPVBSMatrix_Dir * vec4(modelPosition, 1.0);
}